...
#pragma config(Motor,  port9,           rightMotor,    tmotorVex393HighSpeed_MC29, openLoop)
...
task main()
{
	...
	{
		if ((SensorValue[rightEncoder] < 3200))
	{
		...
		{
			// Turn slightly right
			motor[rightMotor] = -55;		    // Right Motor is run at power level 60
			motor[leftMotor]  = -60;		    // Left Motor is run at power level 80
		}
		else	// Only runs if leftEncoder has counted more encoder counts
		{
			// Turn slightly left
			motor[rightMotor] = -60;		    // Right Motor is run at power level 80
			motor[leftMotor]  = -55;		    // Left Motor is run at power level 60
		}
	}
	if (SensorValue(leftEncoder) < 3200)
	{
